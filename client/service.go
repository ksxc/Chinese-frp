...
func (svr *Service) keepControllerWorking() {
	...
	for {
		...
		for {
			xl.Info("尝试重新连接到服务器。。。（你滴网络大大滴问题）")
			conn, session, err := svr.login()
			if err != nil {
				xl.Warn("重新连接到服务器错误: %v, wait %v 再次重试", err, delayTime)
...
func (svr *Service) login() (conn net.Conn, session *fmux.Session, err error) {
	...
	xl.Info("登录服务器成功，获取运行id [%s], 服务器udp端口 [%d] \n 兮辰汉化-来自GitHub  \n https://github.com/ksxc/Chinese-frp", loginRespMsg.RunID, loginRespMsg.ServerUDPPort)
	return
}
