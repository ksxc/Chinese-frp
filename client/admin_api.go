...
func (svr *Service) apiReload(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		res.Code = 400
		res.Msg = err.Error()
		log.Warn("重新加载frpc代理配置错误: %s", res.Msg)
		return
	}
...
func (svr *Service) apiGetConfig(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		res.Code = 400
		res.Msg = err.Error()
		log.Warn("加载frpc配置文件错误: %s", res.Msg)
		return
	}
...
func (svr *Service) apiPutConfig(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		res.Code = 400
		res.Msg = fmt.Sprintf("读取请求正文错误: %v", err)
		log.Warn("%s", res.Msg)
		return
	}
	...
	if err != nil {
		res.Code = 500
		res.Msg = fmt.Sprintf("将内容写入frpc配置文件错误: %v", err)
		log.Warn("%s", res.Msg)
		return
	}
}
